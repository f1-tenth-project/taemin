...
- 차량의 현재 위치에서 `lookahead_distance`만큼 떨어진 경로상의 목표 지점(Goal Point)을 찾습니다.
- 차량의 후륜 중심과 목표 지점 사이의 각도를 계산하여 Ackermann 조향각을 산출합니다.
- **수식:** $\delta = \arctan(\frac{L \cdot 2y}{l_d^2})$

### 2. Dynamic Speed Control
- 경로가 휘어지는 정도(곡률, Curvature)를 계산하여 코너에서는 감속하고 직선에서는 가속합니다.
- **수식:** $v_{ref} = \max(v_{min}, v_{max} - k_{speed} \cdot |\kappa|)$
- 목표 속도에 도달하기 위해 PID 기반의 가속도 제어를 수행합니다.

### 3. Path Planning (Current Status)
- `center_path`, `left_boundary`, `right_boundary`를 모두 구독합니다.
- **현재 로직:** 안전을 위해 `center_path`를 기본 주행 경로(`racing_path`)로 설정하여 주행합니다. (추후 최적 경로 생성 로직 확장 가능)
...
`racing_controller.yaml` 또는 런치 파일에서 다음 파라미터를 수정하여 주행 성능을 튜닝할 수 있습니다.
